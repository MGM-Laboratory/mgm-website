...
type ColumnMeta = { copyHeight: number; copies: number };

/**
 * An infinite, drifting, 3D-tilted wall of tiles: each column loops
 * seamlessly (two-plus rendered copies per column, offset by the loop
 * length) at its own speed, with pointer-follow tilt on the whole plane and
 * a per-tile hover/focus lift. Ported from React Bits' DriftWall.
 */
export default function DriftWall({
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className={styles.inner}>
        {/* eager: each column renders several stacked copies to loop seamlessly, so most tiles start
            genuinely off-screen above/below the visible band; lazy-load only fetches them once the
            drift carries them into view, which can take up to a minute per loop cycle */}
        {/* eslint-disable-next-line @next/next/no-img-element -- infinite-loop tiling duplicates each src several times; next/image's per-instance optimization doesn't fit this pattern */}
        <img
          src={item.image}
          alt={item.title ?? ""}
          loading="eager"
  ...
  return (
    <div
      ref={containerRef}
      className={cn(styles.driftWall, className)}
      style={cssVars}
