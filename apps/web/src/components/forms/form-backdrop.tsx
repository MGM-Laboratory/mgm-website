...
function PatternBlock({
  ...
  const kept = (index: number) => {
    ...
    const fromEdge = corner === "tl" ? col + row : col + (rows - 1 - row);
    ...
    <>
      ...
      {background.pattern !== "none" ? (
        <div
          ...
          style={
            {
              ["--fx-pattern-dim" as string]: Math.max(0.25, 1 - background.dim / 100),
            } as CSSProperties
          }
        >
          <PatternBlock pattern={background.pattern} corner="tl" cols={3} rows={2} offset={0} />
          <PatternBlock pattern={background.pattern} corner="bl" cols={4} rows={3} offset={5} />
        </div>
      ) : null}
