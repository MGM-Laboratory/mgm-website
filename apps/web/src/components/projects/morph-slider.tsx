...
import styles from "./morph-slider.module.css";

/**
 * Ported from React Bits' MorphSlider (https://reactbits.dev) — a stack of
 * images blended by a WebGL fragment shader (procedural noise/ripple/shear/
 * swirl displacement, not a CSS crossfade) driven by a GSAP tween on the
 * shader's own progress uniform. The shader and GSAP plumbing are unchanged
 * from the original; ported to TypeScript (ogl's own published types are
 * missing the `Triangle` export — see src/types/ogl.d.ts) and given an
 * `onIndexChange` callback so a host component can keep its own UI (title,
 * description, CTA) in sync with whichever slide is currently showing,
 * since the original only tracks that state internally.
 */

export type MorphSliderItem = { image: string; caption?: string };
export type MorphSliderTransition = "melt" | "ripple" | "shear" | "swirl";

const TRANSITIONS: Record<MorphSliderTransition, number> = {
  melt: 0,
  ripple: 1,
  shear: 2,
  swirl: 3,
...
class MorphEngine {
  ...
  destroy() {
    ...
    this.textures.forEach((tex) => {
      ...
      if (handle) this.gl.deleteTexture(handle);
    });
...
export type MorphSliderProps = Readonly<{
  items: MorphSliderItem[];
  startIndex?: number;
  transition?: MorphSliderTransition;
  duration?: number;
  ...
  className?: string;
  /** Fires whenever the settled (post-transition) slide changes, including
   * from autoplay/drag, so a host component can mirror its own UI. */
  onIndexChange?: (index: number) => void;
}>;
...
export function MorphSlider({
  ...
  // Kept current via an effect rather than a direct write during render —
  // the engine only ever reads this from event handlers/rAF, never during
  // render itself, so the one-tick-later update is unobservable here.
  useLayoutEffect(() => {
    optsRef.current = {
      transition,
      duration,
  ...
  useEffect(() => {
    ...
    // unstable in this codebase before (see hero/lanyard's Suspense note in
    // docs/animation-system.md) — a browser/environment without a working
    // context should fall back to a static image instead of taking the rest
    // of the page down with it.
    let engine: MorphEngine | undefined;
    try {
      ...
      console.warn(
        "MorphSlider: WebGL initialization failed, falling back to a static image.",
        error,
      );
      // Deferred a tick so this doesn't setState synchronously inside the
      // effect body (which the lint rule flags as a cascading-render risk)
      // — the fallback still swaps in before the next paint either way.
      queueMicrotask(() => setWebglFailed(true));
      return undefined;
    }
    engineRef.current = engine;
    // Deferred a tick for the same reason as the catch branch above — keeps
    // this setState out of the effect body's synchronous call path.
    queueMicrotask(() => setIndex(startIndex));
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (active && el.setPointerCapture) {
        try {
          ...
          // Pointer capture is best-effort — a failed capture still lets the
          // move/up handlers below work via normal event bubbling.
        }
      }
    };
  ...
  if (webglFailed) {
    const fallback = items[startIndex % Math.max(items.length, 1)];
    return (
      <div className={cn(styles.morphSlider, className)} style={{ borderRadius: `${radius}px` }}>
        {fallback?.image ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img alt="" className={styles.fallbackImage} src={fallback.image} />
        ) : null}
      </div>
    );
  }
  ...
  return (
    <div
      className={cn(styles.morphSlider, className)}
      style={
        {
    ...
    >
          ...
          {items.map((item, i) =>
            item.caption ? (
              <span
                ...
                className={cn(styles.captionText, i === index && styles.isActive)}
              >
                {item.caption}
              </span>
            ) : null,
      ...
      {showControls ? (
        <div className={styles.controls}>
          ...
          <button type="button" className={styles.btn} aria-label="Next slide" onClick={handleNext}>
            <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
              <path
                d="M9 5l7 7-7 7"
                fill="none"
