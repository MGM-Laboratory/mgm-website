...
| `pr-bot.yml`                                       | `workflow_run` (checks and the trusted review signal) | Upserts one PR status comment for each completed check workflow; reads submitted reviews through the API and replies to an exact `LGTM` with the GIF. It runs from the default branch, never from PR code.                                              |
...
| `pr-commands.yml`                                  | PR comment created                                    | Exact `LGTM` → GIF, for anyone, no side effects. `/check`, `/preview`, `/merge`, `/close`, gated to OWNER/MEMBER/COLLABORATOR or anyone listed in `CODEOWNERS`.                                                                                         |
...
`ci.yaml`, `e2e.yaml`, `lighthouse.yaml`, `security.yaml`, and both `publish-docker-image-*.yml` callers all start with a `changes` job that calls the reusable `detect-changes.yml`. The classifier in `.github/scripts/change-scope.mjs` emits independent API, web, browser, Lighthouse, prose, dependency, CodeQL, Trivy, and Docker scopes. Each expensive job is gated by its scope, and each workflow ends with an aggregate gate (`Lint, typecheck, test & build`, `E2E gate`, `Lighthouse gate`, `Security gate`, `Prose gate`, or a Docker gate). A PR or push therefore runs the checks relevant to the changed paths while the aggregate check remains present for the branch ruleset.
