...
- **CodeQL** — `js/xss-through-dom` fires on `contact-form.tsx`'s attachment preview (`<img src={url}>` / `<a href={url}>`), where `url` comes from `previewUrls.get(file)`. The query treats the selected `File` as tainted and the JSX `src`/`href` binding as a sink, but `url` is always `URL.createObjectURL(file)`'s return value — an opaque, browser-minted `blob:` reference that can never carry HTML metacharacters or a `javascript:` scheme, regardless of the file's own name/content. A real guard (`isSafeBlobUrl`, checking `url.startsWith("blob:")`) gates both usages, but CodeQL's dataflow doesn't recognize a custom string-prefix predicate as a sanitizing barrier for this query the way it does `Array.isArray`/`Buffer.isBuffer` for the type-confusion query above — so this is suppressed the same way, `// codeql[js/xss-through-dom]` on the line above each flagged element.
