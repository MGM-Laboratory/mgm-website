...
- **`publish-docker-image-latest.yml`** — caller, triggers on push to `main` + `workflow_dispatch`. Calls the image workflow with `tag: latest`, using the local `./...` reference (fine on `push`).
- **`publish-docker-image-staging.yml`** — caller, triggers on any pull request (`branches: ["*"]`) + `workflow_dispatch`. Calls the image workflow with `tag: staging`, using the full `owner/repo/path@main` reference — local `./...` references are broken specifically under the `pull_request` trigger (a documented GitHub Actions bug), so this one can't use the same shorthand as the `latest` caller. This still can't resolve until `publish-docker-image.yml` exists on `main`, same bootstrap requirement any new workflow file has.
