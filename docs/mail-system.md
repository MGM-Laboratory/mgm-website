...
| `ses`    | always "configured" (the AWS SDK client is constructed unconditionally, relying on its default credential chain — an explicit key is optional) | `AWS_REGION` (default `us-east-1`), `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` (optional)                                                                                            |

**Shared sender identity**: `SES_FROM_EMAIL` (must be a bare, valid email — some code paths pass it straight to SES's raw `Source` field) is the address every provider sends from. `MAIL_FROM_NAME`, if also set, is combined as `"${MAIL_FROM_NAME} <${SES_FROM_EMAIL}>"` — a single computed `from` string shared by all three providers' send calls. Neither var is required; with `SES_FROM_EMAIL` unset, `sendEmail()` throws immediately ("SES_FROM_EMAIL is not configured").
