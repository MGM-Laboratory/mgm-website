...
Every internal client-side navigation plays a full-screen transition: the MGM logo covers the screen, the destination page loads behind it, then it reveals. Built across PR #45 and #46. All of it lives in `apps/web/src/components/transition/route-transition.tsx` (`RouteTransition`), mounted in `layout.tsx` as a sibling of `<SiteHeader>` and outside `<SmoothScroll>` — deliberately, so neither the header's stacking context nor the smoother's transform wrapper (see `docs/architecture.md`'s root layout chain) affects it.
...
| Hold   | `engageHoldIfNeeded` | until `routeReady` | A breathing yoyo (`scale: 1.08`, 0.9s, `sine.inOut`, `repeat: -1`) — see the timing note below                                                                      |
