...
2. `<Providers>` (`src/components/providers.tsx`): `next-themes` ThemeProvider (`attribute="class"` → `.dark` on `<html>`) plus other global providers. Everything below is a child of `<Providers>`, in this exact order:
   ...
   3. `<ProjectTransition />` (`src/components/transition/project-transition.tsx`): the project zoom overlay (z-45, below the header) that carries a card's cover into `/projects/[slug]` and back, plus the strip that shields the header while a zoom runs. See `docs/page-transition.md`.
   4. `<SiteHeader>` (`src/components/site-header.tsx`): fixed, `h-16`, z-50, frosted glass. Left: `<LogoMark />` (animated MGM mark). Right: `<ThemeToggle />` and the `<NavMenu />` button. On `/projects/<slug>` a centre Back pill links to `/projects`. The center navbar was deliberately removed. The menu is the only navigation.
   5. `<SmoothScroll>` (`src/components/smooth-scroll.tsx`): GSAP ScrollSmoother wrapper around all page content. **Not created under `prefers-reduced-motion`, and only on `/` (`shouldSmooth = pathname === "/"`)**. Every other route uses native scroll. Scroll locking goes through the owner-counted `lib/scroll-lock.ts` (the menu and the `/projects` intro each hold it under their own owner name): it sets `html { overflow: hidden }`, reserves the scrollbar gutter when a classic scrollbar is showing, pauses `ScrollSmoother`, and notifies JS scrollers (the Lenis instances on `/projects` and on project detail pages) to stop. In dev, the instance is exposed as `window.__smoother`.
...
| `components/sections/showcase-section.tsx`   | A "Netflix row": header plus a horizontally scrolling card track, used by the About page's projects teaser and the articles section                                                                                                                                                                              |
...
| `components/projects/`                       | The `/projects` index (hero with idle play, gated list reveal, WebGL cover stage with a DOM fallback, lusion-style card text) and, under `detail/`, the lusion-style project detail page (horizontal track, WebGL media stage, topography background, next-project hand-off). Full spec: `docs/projects-page.md` |
...
- `src/lib/route-reveal.ts`, `src/lib/projects-intro.ts`, `src/lib/project-transition.ts`: choreography signals (the curtain's reveal, the `/projects` intro and list reveal, the project zoom between the list and a project); see `docs/page-transition.md` and `docs/projects-page.md`.
- `src/lib/project-themes.ts`: the 20 project detail page themes (light and dark palettes, contrast-checked) and the helpers that turn one into CSS variables.
